...
<div class="list-item" id="flightFormBook">
    <div>
        ...
        @foreach($flightsData['arr'] as $keys => $rows)
        <div class="col-md-12 col-lg-12">
            {{-- {{ dd($rows); }} --}}
            @include('Flight::frontend.layouts.search.loop-grid_1')
        </div>
        <input type="hidden" id="flightSrk" value="{{ $flightsData['srk'] }}">
        <input type="hidden" id="flightSearchToken" value="{{ $flightsData['tokensSearchResults'] }}">
        @endforeach
        @else
        <div class="col-lg-12">
            {{__("Flight not found")}}
...
{{-- @if($rows->total() > 0)
            <div class="text-center text-md-left font-size-14 mb-3 text-lh-1">{{ __("Showing :from - :to of :total
                Flights",["from"=>$rows->firstItem(),"to"=>$rows->lastItem(),"total"=>$rows->total()]) }}</div>
@endif
